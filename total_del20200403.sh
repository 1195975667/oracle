...
echo "Input the time you want to delete,for example:7 ,we will delete files 7 dayes before:"
read delete_time
echo "we will delete lofiles before $delete_time days before."
...
## show the user
ps -ef|grep pmon|grep -v grep|awk '{print $1}'>/tmp/user`date +%Y%m%d`.list
cat /tmp/user.list|while read user
do
echo "now user is $user"
## get the log trace
base_dir=`su - $user -c 'echo $ORACLE_BASE'`
echo "base_dir is $base_dir"
su - oracle -c "adrci exec='show homepath'|grep diag>/tmp/path.log"
...
find $base_dir/$line/trace -mtime +$delete_time -name "*.trc" |xargs -n 10 rm
find $base_dir/$line/trace -mtime +$delete_time -name "*.trm" |xargs -n 10 rm
find $base_dir/$line/trace -name "listener*.log" |xargs  rm

cd $base_dir/$line/alert
echo "now directory is in `pwd`"
find $base_dir/$line/alert -mtime +$delete_time -name "*.xml"|xargs -n 10 rm
done
done
...
find $auditdir -mtime +$delete_time -name "*.aud" |xargs -n 10 rm
done
